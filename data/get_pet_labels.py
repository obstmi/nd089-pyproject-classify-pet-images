...
def get_pet_labels(image_dir):
    ...
    """
    # Check if image_dir is valid and exists
    if not image_dir or not path.isdir(image_dir):
        print("Error: Invalid or non-existent directory:", image_dir)
        return {}
    ...
    for idx in range(0, len(in_files), 1):
        ...
        if in_files[idx][0] != ".":
            # Creates temporary label variable to hold pet label name extracted from filename
            pet_label = ""

            # Sets temporary label to filename in lower case letters
